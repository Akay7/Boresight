...
- **WHEN** a client sends a move request with `x=0.5, y=0.5`
...
- **WHEN** a client sends a move request with `x` or `y` outside `[0.0, 1.0]`
...
- **WHEN** a client sends a move request with a non-numeric `x` or `y`
- **THEN** the server responds with a client error (HTTP 422) and does
  not invoke the cursor backend
